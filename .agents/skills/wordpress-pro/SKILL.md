...
|-------|-----------|-----------|
| Theme Development | `references/theme-development.md` | Templates, hierarchy, child themes, FSE, `style.css` headers |
...
WordPress **requires** a `style.css` file in the **theme root** (not a subdirectory). Its header comment registers the theme in **Appearance → Themes**; without valid metadata, WordPress will not list the theme.

- **Required header field:** `Theme Name:` (all themes). **Child themes** must also include `Template: parent-theme-folder` (the parent’s directory slug, case-sensitive).
- **Typical fields:** `Theme URI`, `Author`, `Author URI`, `Description`, `Version`, `Requires at least`, `Tested up to`, `Requires PHP`, `License`, `License URI`, `Text Domain`, `Tags`. The **Theme Directory** uses `Version` for update checks.
- **Custom CSS:** Anything after the closing `*/` is normal CSS. Many themes enqueue this file with `get_stylesheet_uri()` so the active stylesheet (child wins) loads on the front end; classic themes often put most CSS in `assets/` and keep `style.css` lean except for the header.
- **Block themes (FSE):** Global design belongs in **`theme.json`** and the Site Editor; **Styles → Additional CSS** (WordPress 6.2+) covers ad hoc rules. You still need `style.css` for registration—use it for small overrides, `@font-face`, or legacy hooks that expect it.
- **Parent vs child:** `get_template_directory_uri()` / `get_template_directory()` refer to the **parent**; `get_stylesheet_uri()` / `get_stylesheet_directory()` refer to the **active** theme (child if one is active).

Full header examples (classic, child, block layout) live in `references/theme-development.md`.
